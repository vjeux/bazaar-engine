...
fetch("https://cdn.playthebazaar.com/bazaardesigndataprod/cards.json")
  .then((res) => res.text())
  .then((text) => {
    writeFileSync(__dirname + "/json/cards.json", text, "utf-8");
  });

fetch("https://www.howbazaar.gg/api/monsterEncounterDays")
  .then((res) => res.text())
  .then((text) => {
    writeFileSync(__dirname + "/json/monsterEncounterDays.json", text);
  });

fetch("https://www.howbazaar.gg/api/items")
  .then((res) => res.text())
  .then((text) => {
    writeFileSync(__dirname + "/json/items.json", text);
...
fetch("https://www.howbazaar.gg/api/skills")
  .then((res) => res.text())
  .then((text) => {
    writeFileSync(__dirname + "/json/skills.json", text);
