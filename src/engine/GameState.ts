...
import { BoardCard, GameState, Player } from "./engine2/engine2.ts";
import { EventBus } from "./engine2/eventBus.ts";
...
function _createBoardCardFromCard(
  ...
  let attributes = {
    ...baseAttributes,
    Abilities: {} as { [key: string]: Ability },
    AbilityIds: [] as string[],
    AuraIds: [] as string[],
    Auras: {} as { [key: string]: Aura },
  ...
  // Iterate over the tiers in order and replace their attributes until the desired tier is reached.
  ...
  // Add the correct Abilities to the attributes, based on the tier and it's abilityids
  for (const abilityId of attributes.AbilityIds) {
    attributes.Abilities[abilityId] = card.Abilities[abilityId];
  }

  // Add the correct Auras to the attributes, based on the tier and it's auraids
  for (const auraId of attributes.AuraIds) {
...
export function createBoardCardFromId(
  Cards: Cards,
  cardId: string,
  tier: Tier | undefined,
  enchantment: keyof Enchantments | null = null,
    ...
    players: config.map((player) => {
      if (player.type === "monster") {
        ...
        return createBoardPlayer(
          ...
          (player.stash ?? []).map((c) =>
            createBoardCardFromId(Cards, c.cardId, c.tier),
          ),
        );
      }
    }),
