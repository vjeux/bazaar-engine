            ...
            (
                ...
                forEachAbility(targetBoardCard, (ability) => {
                    if (
                        ...
                        subjects.forEach(
                            ([subjectPlayerID, subjectBoardCardID]) => {
                                if (
    ...
    if (action.$type === "TActionPlayerDamage") {
        ...
        targetCards
            ...
            .sort((a, b) => {
                ...
                const amountA =
                    nextGameState.players[a[0]].board[a[1]][tickKey];
                const amountB =
                    nextGameState.players[b[0]].board[b[1]][tickKey];
        ...
        targetCards
            ...
            .forEach(([actionTargetPlayerID, actionTargetBoardCardID]) => {
                ...
                updateCardAttribute(
                    ...
                    "tick",
                    Math.min(
                        nextBoardCard.CooldownMax,
                        nextBoardCard.tick + amount
                    )
                );
            });
    } else if (
...
function getActionValue(
    ...
    if (value.$type === "TFixedValue") {
        ...
        targetCards.forEach(([valueTargetPlayerID, valueTargetBoardCardID]) => {
            amount +=
                gameState.players[valueTargetPlayerID].board[
...
function getTargetCards(
        ...
        if (target.$type === "TTargetCardRandom") {
            ...
            while (currentIndex != 0) {
                let randomIndex = Math.floor(
                    gameState.getRand() * currentIndex
                );
                currentIndex--;
    ...
    if (
        ...
        results.forEach(([testPlayerID, testBoardCardID]) => {
            const boardCard =
                gameState.players[testPlayerID].board[testBoardCardID];
...
function runGameTick(initialGameState) {
    ...
    forEachCard(
        initialGameState,
        (player, playerID, boardCard, boardCardID) => {
    ...
    forEachCard(gameState, (player, playerID, boardCard, boardCardID) => {
        const nextBoardCard =
            nextGameState.players[playerID].board[boardCardID];
                ...
                if ("Multicast" in nextBoardCard) {
                    ...
                    for (let i = 0; i < nextBoardCard.Multicast - 1; ++i) {
                        nextGameState.multicast.push({
                            tick:
                                nextGameState.tick + (i + 1) * MULTICAST_DELAY,
                            playerID,
                            boardCardID,
                        });
                    }
        ...
        >
            ...
            {boardCard.TooltipIds.map((tooltipId, i) => {
                const tooltip = boardCard.card.Localization.Tooltips[
                    ...
                    const action =
                        boardCard.card[type === "aura" ? "Auras" : "Abilities"][
                            id
                        ].Action;
                    if (action.Value) {
    ...
    return (
        <div className="tooltipContainer">
            ...
            >
                ...
                {boardCard.Multicast !== undefined &&
                    boardCard.Multicast > 1 && (
                        <div
                            style={{
                                position: "absolute",
                                left: "50%",
                        ...
                        {player.board
                            ...
                            .map((boardCard, i) => (
                                <BoardSkill
                                    ...
                                    key={i}
                                    playerID={playerID}
                                    boardCardID={player.board.indexOf(
                                        boardCard
                                    )}
                                />
                            ))}
